-- Qualify ledger columns so they cannot conflict with the RETURNS TABLE output names.
-- This preserves the existing JST, distinct-Gate, and once-per-day reward rule.
...
begin
  ...
  if v_reward_granted then
    ...
    values (p_auth_user_id, greatest(v_row_challenge_day, 1), greatest(v_row_check_in_count, 0) + 1, now())
