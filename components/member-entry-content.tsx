...
const memberEntryCopy = {
  jp: {
    ...
    accessNoticeTitle: "このアカウントではBASIC会員情報を確認できませんでした。",
    accessNoticeBody: "決済時と同じメールアドレスでログインしているかご確認ください。",
    accessNoticeSupport: "解決しない場合は、LINEサポートへご連絡ください。",
    planLabel: "現在の段階",
    planLabels: {
      free: "Free",
...
export function MemberEntryContent({
  ...
  const [memberLoginHref, setMemberLoginHref] = useState("/login");
  const [requestedNextPath, setRequestedNextPath] = useState("");
          ...
          <div className="mx-auto mt-2 max-w-5xl space-y-6">
            {showAccessNotice ? (
              <section className="rounded-[24px] border border-[#f1d7a1]/20 bg-[linear-gradient(180deg,rgba(212,186,117,0.10),rgba(255,255,255,0.02))] px-5 py-5 sm:px-6">
                ...
                <h2 className="mt-3 text-xl font-semibold text-white">{copy.accessNoticeTitle}</h2>
                <p className="mt-3 text-sm leading-7 text-white/74">{copy.accessNoticeBody}</p>
                <p className="mt-2 text-sm leading-7 text-white/60">{copy.accessNoticeSupport}</p>
        ...
        {!showMemberLoadingState && effectiveIsLoggedIn ? (
          ...
          <>
            <div className="mx-auto mt-8 hidden max-w-4xl rounded-[28px] border border-[#f1d7a1]/18 bg-[radial-gradient(circle_at_top,rgba(212,186,117,0.12),transparent_30%),linear-gradient(180deg,rgba(255,255,255,0.05),rgba(255,255,255,0.02))] p-5 sm:block sm:p-6">
              ...
              <div className="mt-5 flex flex-col gap-3 sm:flex-row sm:items-center">
                ...
                <a
                  ...
                  target="_blank"
                  rel="noopener noreferrer"
                  className="inline-flex min-h-[56px] items-center justify-center rounded-full border border-white/12 bg-white/[0.04] px-6 py-3 text-sm font-semibold text-white transition duration-300 hover:bg-white/[0.08]"
                >
                  {copy.actions.line}
                </a>
              </div>
            ...
            <div className="mx-auto mt-4 max-w-4xl rounded-[24px] border border-white/10 bg-white/[0.03] p-5">
              <p className="text-sm leading-7 text-white/78">{copy.fallback}</p>
