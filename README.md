...
`RESEND_FROM_EMAIL` 이 비어 있으면 기본값으로 `Meisoulife <onboarding@resend.dev>` 를 사용합니다. `hello@meisoulife.com` 발신 도메인이 Resend에서 검증된 뒤에는 `Meisoulife <hello@meisoulife.com>` 로 바꾸세요.
...
테스트 결제 후 Vercel Logs 기대값:

```text
[stripe-webhook] checkout.session.completed received
[stripe-webhook] customer email found
[resend] payment confirmation email sent successfully
```
